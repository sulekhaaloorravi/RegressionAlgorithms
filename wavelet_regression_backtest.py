...
UPDATED VERSION with 3 upgrades:

1) Posterior-weighted expert training (soft regimes) via sample_weight
2) Proper mixture quantiles using residual-sampling at the distribution level
3) Walk-forward backtest (end-to-end, leakage-safe regime ordering) + MAE + directional accuracy

Assumptions:
- Your CSV has: date column + target column + optional numeric feature columns
- Quantiles SHOULD include 0.50 (recommended), because we always output pred_p50.
"""
...
class MoEConfig:
    ...
    # Mixture quantile sampling
    mixture_samples: int = 3000
    residual_pool_cap: int = 5000
    ...
    bt_initial_train_size: int = 140
    bt_step: int = 1
    bt_eval_h: int = 1
    bt_out_csv: str = "backtest_walkforward.csv"
...
def build_dataset(
    ...
    *,
    ...
    """
    if lookback <= 0:
        raise ValueError("lookback must be > 0 for wavelet/STFT windowing.")
    ...
    for win in rolling_windows:
        # target rolling
        rf = rolling_features(pd.Series(base["target_t"]), win).add_prefix("tgt_")
...
def order_regimes_by_stress(ds: pd.DataFrame, post: np.ndarray, n_regimes: int):
    """
    Reorder regimes low->high stress using mean abs(target_t).
    Returns mapping old->new and reordered posteriors.
    """
...
# =========================
# TRAIN EXPERTS (posterior-weighted per regime)
# =========================
def train_experts_weighted(
    ds: pd.DataFrame,
    post: np.ndarray,
    *,
    horizon: int,
    ...
    """
    Posterior-weighted experts:
      - each regime k is trained on ALL rows
      - sample_weight = post[:, k]
    """
    if 0.50 not in set(float(q) for q in quantiles):
        # Not strictly required, but strongly recommended because we always output pred_p50.
        print("[WARN] quantiles does not include 0.50. Recommended to include 0.50 for consistent 'pred_p50'.")
    ...
    for k in range(n_regimes):
        w = post[:, k].astype(float)

        # ignore near-zero weights for efficiency
        mask = w > 1e-6
        Xk = X.iloc[mask].reset_index(drop=True)
        Yk = Y[mask, :]
        wk = w[mask]

        eff_n = (wk.sum() ** 2) / (np.sum(wk ** 2) + 1e-12)  # effective sample size
        name = regime_names[k] if k < len(regime_names) else f"REGIME_{k}"
        print(f"\nTraining weighted expert regime {k} ({name}) | rows={len(Xk)} | eff_n≈{eff_n:,.1f}")
        ...
        for h in range(horizon):
            ...
            # Optional weighted CV diagnostics (time order preserved)
            if len(Xk) >= 300:
                maes = []
                for tr, te in cv.split(Xk):
                    med.fit(Xk.iloc[tr], Yk[tr, h], sample_weight=wk[tr])
                    pred = med.predict(Xk.iloc[te])
                    maes.append(mean_absolute_error(Yk[te, h], pred, sample_weight=wk[te]))
                if (h + 1) in [1, 5, 10, 20, 30]:
                    print(f"  t+{h+1:02d} weighted median MAE: {np.mean(maes):,.4f}")

            med.fit(Xk, Yk[:, h], sample_weight=wk)
            ...
            for q in quantiles:
                ...
                qb.fit(Xk, Yk[:, h], sample_weight=wk)
...
# =========================
# RESIDUAL POOLS (for mixture-quantile sampling)
# =========================
def build_residual_pools(
    ...
    *,
    horizon: int,
    n_regimes: int,
    max_pool_per_regime: int = 5000,
    seed: int = 42,
):
    """
    Build residual pools per regime per horizon:
      residual = (y_true - median_pred_regime)
    Uses hard labels ONLY for pooling (experts were trained weighted).
    """
    ...
    labels = post_ord.argmax(axis=1)
    pools: Dict[int, Dict[int, List[float]]] = {k: {h: [] for h in range(horizon)} for k in range(n_regimes)}
    ...
    for k in range(n_regimes):
        ...
        if idx.size > max_pool_per_regime:
            idx = rng.choice(idx, size=max_pool_per_regime, replace=False)

        Xk = X.iloc[idx]
        for h in range(horizon):
            pred = experts[k]["median"][h].predict(Xk)
            res = (Y[idx, h] - pred).astype(float)
            res = res[np.isfinite(res)]
            if res.size:
                pools[k][h] = res.tolist()

    return pools


# =========================
# FORECAST (proper mixture quantiles via sampling)
# =========================
def forecast_horizon_mixture_sampling(
    ds: pd.DataFrame,
    experts: dict,
    residual_pools: dict,
    p_regime: np.ndarray,
    *,
    ...
    """
    Distribution-level mixture:
      draw samples from regime-specific predictive distributions:
        y = median_pred_k + residual_sample_k
      mix regimes using p_regime
      compute empirical quantiles
    """
    q_levels = sorted(set(float(q) for q in quantiles))
    if 0.50 not in q_levels:
        q_levels.append(0.50)
    ...
    # Precompute per-regime median forecasts for all horizons
    med_kh = np.zeros((n_regimes, horizon), dtype=float)
    for k in range(n_regimes):
        for h in range(horizon):
            med_kh[k, h] = float(experts[k]["median"][h].predict(X_last)[0])

    # Normalize regime probs
    p = np.clip(np.asarray(p_regime, dtype=float), 0.0, 1.0)
    p = p / (p.sum() + 1e-12)

    # Draw regime indices according to p
    reg_draws = rng.choice(np.arange(n_regimes), size=int(n_samples), p=p)

    # Output quantile lists
    out_q: Dict[float, List[float]] = {q: [] for q in q_levels}

    for h in range(horizon):
        samples = np.empty(int(n_samples), dtype=float)

        # vectorized residual draw per regime
        for k in range(n_regimes):
            pos = np.where(reg_draws == k)[0]
            if pos.size == 0:
                continue

            pool = residual_pools.get(k, {}).get(h, [])
            if pool:
                pool_arr = np.asarray(pool, dtype=float)
                ridx = rng.randint(0, len(pool_arr), size=pos.size)
                res = pool_arr[ridx]
            else:
                res = np.zeros(pos.size, dtype=float)

            samples[pos] = med_kh[k, h] + res

        # empirical quantiles for this horizon
        for q in q_levels:
            out_q[q].append(float(np.quantile(samples, q)))

    start = pd.to_datetime(ds["date"].iloc[-1]) + pd.Timedelta(days=1)
    dates = pd.bdate_range(start, periods=horizon)

    out = pd.DataFrame({"date": dates})
    out["pred_p50"] = out_q[0.50]

    for q in q_levels:
        if q == 0.50:
            continue
        out[f"pred_p{int(round(q*100)):02d}"] = out_q[q]
...
def walk_forward_backtest(
    ...
    *,
    ...
    """
    End-to-end walk-forward (leakage-safe):
      - train on raw[:train_end]
      - build ds_train
      - fit regime model on ds_train
      - order regimes on ds_train only
      - train weighted experts on ds_train
      - build residual pools on ds_train
      - forecast next horizon from last ds_train row
      - evaluate pred_p50 at eval_h against realized raw target

    Returns:
      bt_df, metrics dict
    """
    if eval_h < 1 or eval_h > horizon:
        raise ValueError("eval_h must be between 1 and horizon")

    results = []
    n_raw = len(raw)

    # Ensure numeric target for realized series
    raw2 = raw.copy()
    raw2[target_col] = pd.to_numeric(raw2[target_col], errors="coerce")

    for train_end in range(initial_train_size, n_raw - horizon - 1, step):
        train_raw = raw2.iloc[:train_end].reset_index(drop=True)
        ...
        kind, reg_model, scaler, post, used_cols = fit_regime_model(
            ...
            regime_feature_cols_explicit=regime_feature_cols_explicit,
        )

        mapping, post_ord = order_regimes_by_stress(ds_train, post, n_regimes)

        experts = train_experts_weighted(
            ds_train,
            post_ord,
            horizon=horizon,
            n_regimes=n_regimes,
        ...
        residual_pools = build_residual_pools(
            ds_train,
            experts,
            post_ord,
            horizon=horizon,
            n_regimes=n_regimes,
            max_pool_per_regime=residual_pool_cap,
            seed=42,
        )
        ...
        fc = forecast_horizon_mixture_sampling(
            ds_train,
            experts,
            residual_pools,
            p_last,
        ...
        pred = float(fc["pred_p50"].iloc[eval_h - 1])

        # Realized at train_end+eval_h, previous at train_end (last known)
        y_true = raw2[target_col].iloc[train_end + eval_h]
        y_prev = raw2[target_col].iloc[train_end]

        if pd.notna(y_true) and pd.notna(y_prev) and np.isfinite(pred):
            y_true_f = float(y_true)
            y_prev_f = float(y_prev)

            results.append(
                {
                    "train_end_date": raw2[date_col].iloc[train_end],
                    "eval_date": raw2[date_col].iloc[train_end + eval_h],
                    "y_prev": y_prev_f,
                    "y_true": y_true_f,
                    "pred_p50": pred,
                    "abs_error": abs(y_true_f - pred),
                    "signed_error": (y_true_f - pred),
                    "dir_true": float(np.sign(y_true_f - y_prev_f)),
                    "dir_pred": float(np.sign(pred - y_prev_f)),
                    "dir_hit": float(np.sign(y_true_f - y_prev_f) == np.sign(pred - y_prev_f)),
                }
            )
    ...
    if bt_df.empty:
        raise ValueError(
            "Backtest produced no results. Increase data length or adjust initial_train_size/step/eval_h."
        )

    mae = float(bt_df["abs_error"].mean())
    dir_acc = float(bt_df["dir_hit"].mean())
    metrics = {"MAE": mae, "Directional_Accuracy": dir_acc, "N": int(len(bt_df))}
...
class GenericMoEForecaster:
    def __init__(self, config: MoEConfig):
        ...
        self.residual_pools_: Optional[dict] = None
    ...
    def fit(self) -> "GenericMoEForecaster":
        ...
        self.experts_ = train_experts_weighted(
            ds,
            post_ord,
            horizon=cfg.horizon,
            n_regimes=cfg.n_regimes,
        ...
        self.residual_pools_ = build_residual_pools(
            ds,
            self.experts_,
            post_ord,
            horizon=cfg.horizon,
            n_regimes=cfg.n_regimes,
            max_pool_per_regime=cfg.residual_pool_cap,
            seed=42,
        )
        return self

    def predict(self) -> pd.DataFrame:
        if self.ds_ is None or self.experts_ is None or self.residual_pools_ is None:
        ...
        fc = forecast_horizon_mixture_sampling(
            self.ds_,
            self.experts_,
            self.residual_pools_,
            p_last,
            horizon=cfg.horizon,
            n_regimes=cfg.n_regimes,
            quantiles=cfg.quantiles,
            n_samples=cfg.mixture_samples,
            seed=42,
        )
...
# Run Model
# =========================
def run_model(csv_path: str, date_col: str, target_col: str, feature_cols: Optional[List[str]] = None):
    cfg = MoEConfig(
        csv_path=csv_path,
        date_col=date_col,
        target_col=target_col,
        ...
        regime_feature_mode="auto",  # or "explicit"
        # regime_feature_cols_explicit=["target_t", "r20_tgt_std_20", "feat_some_retlike_t"],
        quantiles=(0.10, 0.50, 0.90),
        cv_splits=5,
        mixture_samples=3000,
        residual_pool_cap=5000,
        do_backtest=True,
        bt_initial_train_size=600,
        bt_step=5,
        bt_eval_h=1,
    )
    ...
    if cfg.do_backtest:
        raw = load_csv_generic(csv_path, date_col)
        bt_df, metrics = walk_forward_backtest(
            raw,
            date_col=date_col,
            target_col=target_col,
            feature_cols=feature_cols,
            ...
            quantiles=cfg.quantiles,
            cv_splits=cfg.cv_splits,
            residual_pool_cap=cfg.residual_pool_cap,
            mixture_samples=2000,  # slightly lower for speed in backtest
            initial_train_size=cfg.bt_initial_train_size,
            step=cfg.bt_step,
            eval_h=cfg.bt_eval_h,
        )
        bt_df.to_csv(cfg.bt_out_csv, index=False)
        print("\nBacktest metrics:", metrics)
        print(f"Saved: {cfg.bt_out_csv}")
